...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/$(CUSTOM_PRODUCT_DIR)/vars/aosp_target_release
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/$CUSTOM_PRODUCT-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
...
function repopick() {
    T=$(gettop)
    $T/$(CUSTOM_PRODUCT_DIR)/build/tools/repopick.py $@
}
