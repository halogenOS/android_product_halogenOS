...
ifeq "$(wildcard $(KERNEL_SRC) )" ""
    ...
    ifeq ($(TARGET_KERNEL_CONFIG)$(TARGET_KERNEL_CONFIG_EXT),)
        ...
        ifneq ($(TARGET_FORCE_PREBUILT_KERNEL),)
            $(error "PREBUILT KERNEL IS NOT ALLOWED!")
        else
...
$(TARGET_PREBUILT_INT_KERNEL): $(DEPMOD) $(DTC) | $(KERNEL_CONFIG) $(KERNEL_MODULES_PARTITION_FILE_LIST) $(SYSTEM_KERNEL_MODULES_PARTITION_FILE_LIST)
	...
	$(hide) if grep -q '=m' $(KERNEL_CONFIG); then \
			...
			$(if $(SYSTEM_KERNEL_MODULES),\
				...
				[ $$? -ne 0 ] && exit 1; \
				($(call build-image-kernel-modules-custom,$$gki_modules,$(SYSTEM_KERNEL_MODULES_OUT),$(SYSTEM_KERNEL_MODULE_MOUNTPOINT)/,$(SYSTEM_KERNEL_DEPMOD_STAGING_DIR),$(SYSTEM_KERNEL_MODULES_LOAD),$(GKI_SUFFIX),$(SYSTEM_KERNEL_MODULES_PARTITION_FILE_LIST),)) || exit "$$?"; \
...
# BUILT_DTBOIMAGE is set in BoardConfigKernel.mk at config time

$(DTBO_OUT):
	mkdir -p $(DTBO_OUT)

ifeq ($(BOARD_USES_QCOM_MERGE_DTBS_SCRIPT),true)
$(BUILT_DTBOIMAGE): $(INSTALLED_DTBIMAGE_TARGET)
else
$(BUILT_DTBOIMAGE): $(DTBO_OUT)
endif
$(BUILT_DTBOIMAGE): $(DTC) $(MKDTIMG) $(MKDTBOIMG)
$(BUILT_DTBOIMAGE):
...
ifeq ($(BOARD_USES_QCOM_MERGE_DTBS_SCRIPT),true)
	...
	PATH=$(abspath $(HOST_OUT_EXECUTABLES)):$${PATH} python3 $(BUILD_TOP)/$(CUSTOM_PRODUCT_DIR)/build/tools/merge_dtbs.py --base $(DTBS_BASE) --techpack $(DTB_OUT)/arch/$(KERNEL_ARCH)/boot/dts/vendor/qcom --out $(DTBS_OUT)
